const courses = [
    {
        subject: 'CSE',
        number: 110,
        title: 'Introduction to Programming',
        credits: 2,
        certificate: 'Web and Computer Programming',
        description: 'This course will introduce students to programming. It will introduce the building blocks of programming languages (variables, decisions, calculations, loops, array, and input/output) and use them to solve problems.',
        technology: [
            'Python'
        ],
        completed: false
    },
    {
        subject: 'WDD',
        number: 130,
        title: 'Web Fundamentals',
        credits: 2,
        certificate: 'Web and Computer Programming',
        description: 'This course introduces students to the World Wide Web and to careers in web site design and development. The course is hands on with students actually participating in simple web designs and programming. It is anticipated that students who complete this course will understand the fields of web design and development and will have a good idea if they want to pursue this degree as a major.',
        technology: [
            'HTML',
            'CSS'
        ],
        completed: false
    },
    {
        subject: 'CSE',
        number: 111,
        title: 'Programming with Functions',
        credits: 2,
        certificate: 'Web and Computer Programming',
        description: 'CSE 111 students become more organized, efficient, and powerful computer programmers by learning to research and call functions written by others; to write, call , debug, and test their own functions; and to handle errors within functions. CSE 111 students write programs with functions to solve problems in many disciplines, including business, physical science, human performance, and humanities.',
        technology: [
            'Python'
        ],
        completed: false
    },
    {
        subject: 'CSE',
        number: 210,
        title: 'Programming with Classes',
        credits: 2,
        certificate: 'Web and Computer Programming',
        description: 'This course will introduce the notion of classes and objects. It will present encapsulation at a conceptual level. It will also work with inheritance and polymorphism.',
        technology: [
            'C#'
        ],
        completed: false
    },
    {
        subject: 'WDD',
        number: 131,
        title: 'Dynamic Web Fundamentals',
        credits: 2,
        certificate: 'Web and Computer Programming',
        description: 'This course builds on prior experience in Web Fundamentals and programming. Students will learn to create dynamic websites that use JavaScript to respond to events, update content, and create responsive user experiences.',
        technology: [
            'HTML',
            'CSS',
            'JavaScript'
        ],
        completed: false
    },
    {
        subject: 'WDD',
        number: 231,
        title: 'Frontend Web Development I',
        credits: 2,
        certificate: 'Web and Computer Programming',
        description: 'This course builds on prior experience with Dynamic Web Fundamentals and programming. Students will focus on user experience, accessibility, compliance, performance optimization, and basic API usage.',
        technology: [
            'HTML',
            'CSS',
            'JavaScript'
        ],
        completed: false
    }
]

const date = new Date();
let currentyear = date.getFullYear();
document.getElementById("currentyear").innerHTML = currentyear;

let lastModified = document.lastModified;
document.getElementById("lastModified").innerHTML = lastModified;

document.getElementById('hamburguer').addEventListener('click', function(){
    var navLinks = document.getElementById('nav-links');
   navLinks.classList.toggle('active');
});


// Function to display courses and individual course buttons
function displayCourses() {
    const container = document.getElementById('course-container');
    const buttonContainer = document.getElementById('course-buttons');

    container.innerHTML = ''; // Clear container before populating
    buttonContainer.innerHTML = ''; // Clear buttons

    courses.forEach((course, index) => {
        // Create a div for each course
        const courseDiv = document.createElement('div');
        courseDiv.classList.add('course');
        courseDiv.dataset.subject = course.subject;  // Store subject for filtering
        courseDiv.id = `course-${index}`; // Unique ID for each course

        // Add completed class if the course is completed
        if (course.completed) {
            courseDiv.classList.add('completed');
        }

        // Insert course content
        courseDiv.innerHTML = `
            <h2>${course.subject} ${course.number} - ${course.title}</h2>
            <p><strong>Certificate:</strong> ${course.certificate}</p>
            <p><strong>Credits:</strong> ${course.credits}</p>
            <p><strong>Description:</strong> ${course.description}</p>
            <p><strong>Technologies:</strong> ${course.technology.join(', ')}</p>
        `;

        // Append the courseDiv to the container
        container.appendChild(courseDiv);

        // Create a button for each course to toggle its visibility
        const toggleButton = document.createElement('button');
        toggleButton.textContent = `Toggle ${course.subject} ${course.number}`;
        toggleButton.onclick = () => toggleCourse(index);

        // Append the button to the buttonContainer
        buttonContainer.appendChild(toggleButton);
    });
}

// Function to toggle visibility for a specific course
function toggleCourse(index) {
    const courseDiv = document.getElementById(`course-${index}`);
    courseDiv.style.display = (courseDiv.style.display === 'none' || !courseDiv.style.display) ? 'block' : 'none';
}

// Toggle visibility for all courses
function toggleAllCourses() {
    const courses = document.querySelectorAll('.course');
    courses.forEach(course => {
        course.style.display = (course.style.display === 'none' || !course.style.display) ? 'block' : 'none';
    });
}

// Toggle visibility for filtered courses by subject
function filterCourses(subject) {
    const courses = document.querySelectorAll('.course');
    courses.forEach(course => {
        if (course.dataset.subject === subject) {
            course.style.display = (course.style.display === 'none' || !course.style.display) ? 'block' : 'none';
        } else {
            course.style.display = 'none';
        }
    });
}

// Call the function to display the courses when the page loads
window.onload = displayCourses;